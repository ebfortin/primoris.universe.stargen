	...
	public class Accrete : IBodyFormationAlgorithm
	{
										...
										Ratio ecc)
		{
			...
			while (_dustLeft)
			{
				Length a;
				Ratio e;
				a = double.IsNaN(semiMajorAxisAU.Value) ? Length.FromAstronomicalUnits(Utilities.RandomNumber(planet_inner_bound.AstronomicalUnits, planet_outer_bound.AstronomicalUnits)) : semiMajorAxisAU;
				e = double.IsNaN(ecc.Value) ? Ratio.FromDecimalFractions(Utilities.RandomEccentricity()) : ecc;
